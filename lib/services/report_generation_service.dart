...
class ReportGenerationService {
    ...
    if (completedLoans.isNotEmpty) {
      sections.add(ReportSection(
        ...
        sectionTotals: {
          ...
          'outstanding': completedLoans.fold(0, (sum, loan) => sum + loan.remainingAmount),
        },
      ));
    }
    ...
    if (overdueLoans.isNotEmpty) {
      sections.add(ReportSection(
        ...
        sectionTotals: {
          ...
          'outstanding': overdueLoans.fold(0, (sum, loan) => sum + loan.remainingAmount),
        },
      ));
    }
  ...
  Future<Report> _generateDetailedReport(ReportDateFilter filter, String? borrowerFilter) async {
    // Load all loans within the date filter
    List<Loan> loans = await _getFilteredLoans(filter, borrowerName: borrowerFilter);
    ...
    for (var loan in loans) {
      ...
      sections.add(ReportSection(
        title: '${loan.borrowerName} (${loan.status == LoanStatus.active ? "Active" : "Completed"})',
  ...
  Future<Report> _generateOverdueReport(ReportDateFilter filter) async {
    ...
    loans = loans.where((loan) => 
        loan.status == LoanStatus.active && loan.isOverdue).toList();
      ...
      if (loanList.isNotEmpty) {
        sections.add(ReportSection(
          ...
          sectionTotals: {
            ...
            'totalAmount': loanList.fold(0, (sum, loan) => sum + loan.loanAmount),
            'outstandingAmount': loanList.fold(0, (sum, loan) => sum + loan.remainingAmount),
          },
        ));
      }
    });
  ...
  Future<Report> _generateBorrowerReport(ReportDateFilter filter) async {
    ...
    for (var borrower in sortedBorrowers) {
      ...
      final totalAmount = borrowerLoanList.fold(0.0, (sum, loan) => sum + loan.loanAmount);
      final paidAmount = borrowerLoanList.fold(0.0, (sum, loan) => sum + loan.paidAmount);
      final outstandingAmount = totalAmount - paidAmount;
      final activeLoans = borrowerLoanList.where((loan) => loan.status == LoanStatus.active).length;
      final completedLoans = borrowerLoanList.where((loan) => loan.status == LoanStatus.completed).length;
      final overdueLoans = borrowerLoanList.where((loan) => loan.isOverdue).length;
      
      // Create a BorrowerReportItem
      final borrowerItem = BorrowerReportItem(
        borrowerName: borrower,
        whatsappNumber: borrowerLoanList.first.whatsappNumber, // Assuming same number for all loans
      ...
      sections.add(ReportSection(
        title: borrower,
        items: [borrowerItem] + borrowerLoanList.map((loan) => LoanReportItem(loan: loan)).toList(),
        sectionTotals: {
          'totalAmount': totalAmount,
          'paidAmount': paidAmount,
          'outstandingAmount': outstandingAmount,
  ...
  Future<Report> _generateDateReport(ReportDateFilter filter) async {
    ...
    // Define date range
    DateTime startDate = filter.startDate ?? DateTime(2000);  // Default to past date if not specified
    DateTime endDate = filter.endDate ?? DateTime.now();      // Default to today if not specified
    ...
    for (var loan in loans) {
      ...
      if (!monthlyData.containsKey(monthKey)) {
        monthlyData[monthKey] = DateGroupData(
          startDate: month,
          endDate: DateTime(month.year, month.month + 1, 0), // Last day of month
        );
      }
    ...
    for (var loan in loans) {
      ...
      for (var payment in payments) {
        final month = DateTime(payment.paymentDate.year, payment.paymentDate.month, 1);
        final monthKey = DateFormat('yyyy-MM').format(month);
    ...
    monthlyData.forEach((key, data) {
      final quarterKey = '${data.startDate.year} Q${(data.startDate.month - 1) ~/ 3 + 1}';
    ...
    final sortedQuarters = quarterlyGroups.keys.toList()..sort((a, b) => b.compareTo(a)); // Most recent first
    
    for (var quarter in sortedQuarters) {
      final quarterItems = quarterlyGroups[quarter]!;
      quarterItems.sort((a, b) => b.startDate.compareTo(a.startDate)); // Sort by most recent month
      
      sections.add(ReportSection(
        title: quarter,
        items: quarterItems,
        sectionTotals: {
          'totalLent': quarterItems.fold(0.0, (sum, item) => sum + item.totalLent),
          'totalReceived': quarterItems.fold(0.0, (sum, item) => sum + item.totalReceived),
          'newLoans': quarterItems.fold(0.0, (sum, item) => sum + item.newLoans),
          'completedLoans': quarterItems.fold(0.0, (sum, item) => sum + item.completedLoans),
        },
      ));
    }
  ...
  Future<Report> _generatePaymentFlowReport(ReportDateFilter filter) async {
    ...
    for (var month in sortedMonths) {
      ...
      final monthlyItem = DateGroupReportItem(
        ...
        totalReceived: monthlyPayments.fold(0.0, (sum, payment) => sum + payment.paymentAmount),
        newLoans: 0, // Will calculate later
        completedLoans: 0, // Will calculate later
      );
      
      // Calculate loans issued in this month
      final loansThisMonth = loans.where((loan) => 
          loan.loanDate.year == date.year && 
          loan.loanDate.month == date.month).toList();
      
      // Calculate completed loans in this month
      final completedThisMonth = loans.where((loan) => 
          loan.status == LoanStatus.completed && 
          loan.updatedAt.year == date.year && 
          loan.updatedAt.month == date.month).toList();
      ...
      final updatedMonthlyItem = DateGroupReportItem(
        ...
        totalLent: loansThisMonth.fold(0.0, (sum, loan) => sum + loan.loanAmount),
      ...
      sections.add(ReportSection(
        ...
        sectionTotals: {
          ...
          'netFlow': updatedMonthlyItem.totalReceived - updatedMonthlyItem.totalLent,
  ...
  Future<List<Loan>> _getFilteredLoans(
    ...
    if (borrowerName != null && borrowerName.isNotEmpty) {
      loans = loans.where((loan) => 
          loan.borrowerName.toLowerCase().contains(borrowerName.toLowerCase())).toList();
    }

    // Apply date filter if using custom range
    if (filter.useCustomRange && filter.startDate != null && filter.endDate != null) {
      loans = loans.where((loan) {
  ...
  Future<ReportSummary> _calculateReportSummary(List<Loan> loans, ReportDateFilter filter) async {
    final totalLoanAmount = loans.fold(0.0, (sum, loan) => sum + loan.loanAmount);
    final totalPaidAmount = loans.fold(0.0, (sum, loan) => sum + loan.paidAmount);
    ...
    final overdueLoans = loans.where((loan) => 
        loan.status == LoanStatus.active && loan.isOverdue).length;
