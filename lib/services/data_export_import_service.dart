...
class DataExportImportService {
  ...
  Future<String?> exportAllDataToCSV() async {
    try {
      ...
      File paymentFile =
          File('${exportDir.path}/loanbuddy_payments_$timestamp.csv');
  ...
  Future<String> importLoansFromCSV() async {
    try {
      ...
      if (headers.contains('Borrower Name') &&
          headers.contains('Loan Amount')) {
        return await _importLoans(rowsAsListOfValues);
      }
      // Check if it's a payments file
      else if (headers.contains('Loan ID') &&
          headers.contains('Payment Amount')) {
        ...
        try {
          ...
          final loan = Loan(
            ...
            status: row[7].toString() == LoanStatus.completed.name
                ...
                : LoanStatus.active,
            createdAt:
                DateFormat('yyyy-MM-dd HH:mm:ss').parse(row[8].toString()),
            updatedAt:
                DateFormat('yyyy-MM-dd HH:mm:ss').parse(row[9].toString()),
          );
      ...
      for (int i = 1; i < rows.length; i++) {
        try {
          ...
          final payment = Payment(
            ...
            createdAt:
                DateFormat('yyyy-MM-dd HH:mm:ss').parse(row[5].toString()),
          );
